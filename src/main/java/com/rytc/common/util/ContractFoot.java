 ...
 */
public class ContractFoot extends PdfPageEventHelper {
    /**
     * 页眉
     */
    ...
    public void onEndPage(PdfWriter writer, Document document) {
        try {
            ...
            if (fontDetail == null) {
                fontDetail = new Font(bf, 8, Font.NORMAL);// 数据体字体
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        // 1.写入页眉
        ColumnText.showTextAligned(writer.getDirectContent(), Element.ALIGN_LEFT, new Phrase(header, fontDetail), document.right() - 200, document.top() + 20, 0);
    ...
    public void onCloseDocument(PdfWriter writer, Document document) {
        ...
        total.beginText();
        total.setFontAndSize(bf, 8);// 生成的模版的字体、颜色
